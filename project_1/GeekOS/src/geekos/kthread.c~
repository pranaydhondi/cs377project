...
/*static*/ void Setup_User_Thread(
    ...
    //    Print("start pointer %lu, %lu\n", kthread->esp, *((ulong_t*)(kthread->esp)));
 ...
 * Set up the a user mode thread.
 */
/*static*/ void Setup_User_SUC_Thread(
                                     struct Kernel_Thread *kthread,
                                     struct User_Context *userContext,
				     ulong_t eip) {
    extern int userDebug;
 ...
 * Returns pointer to the new thread if successful, null otherwise.
 */
struct Kernel_Thread *Start_User_SUC_Thread(struct User_Context *userContext,
					    bool detached,
					    ulong_t eip) {
    struct Kernel_Thread *kthread = Create_Thread(PRIORITY_USER, detached);
    if (kthread != 0) {
        /* Set up the thread, and put it on the run queue */
      Setup_User_SUC_Thread(kthread, userContext, eip);
        Make_Runnable_Atomic(kthread);
    }
