 ...
 */
static int Sys_PrintEsp(struct Interrupt_State *state) {
  Print("Thread Name is %s\n", CURRENT_THREAD->userContext->name);
  Print("Context entryaddr is -> %lu \n", CURRENT_THREAD->userContext->entryAddr);
  Print("Value is -> %lu\n", (*((ulong_t*)311272)));
  return (int)(*((ulong_t*)311276));
}
 ...
 * Create a new user process.
  * Returns: pid of process if successful, error code (< 0) otherwise
 */
static int Sys_SpawnThread(struct Interrupt_State *state) {
    int rc;
    ulong_t eip = (ulong_t)(state->ebx);
    
    struct Kernel_Thread *process;
    Enable_Interrupts();
    process = Start_User_SUC_Thread(CURRENT_THREAD->userContext, false, eip);
    rc = process->pid;
    Disable_Interrupts();
    return rc;
}
...
static int Sys_Wait(struct Interrupt_State *state) {
  int exitCode, i;
    ...
    //    for (i = 0; i <= 1; i++){
    //  Print("shell.exe esp eax val = %lu, location = %d\n", *((ulong_t*)(311264 - 4*i)), 311264-4*i);
    //}
    // 311264
    // 65504
    return exitCode;
