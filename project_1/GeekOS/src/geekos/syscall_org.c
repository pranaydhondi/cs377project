...
static int Sys_getnewtod(struct Interrupt_State *state) {
    int elapsed_time;
    elapsed_time = g_numTicks;
    if (!Copy_To_User(state->ebx, &elapsed_time, sizeof(int)))
        return -1;
    return 0;
}
static int Sys_filesno(struct Interrupt_State *state) {
	int no_files;
	no_files = CURRENT_THREAD->files_no;
	if (!Copy_To_User(state->ebx, &no_files, sizeof(int)))
        return -1;
    return 0;
}
/*
